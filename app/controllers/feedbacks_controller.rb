class FeedbacksController < ApplicationController
  ...
  before_action :check_feedback_submission, only: %i[new create]
  before_action :set_feedback, only: [:destroy]
  before_action :check_admin_role, only: %i[index destroy]
  ...
  def check_feedback_submission
    return unless Feedback.exists?(user_id: current_user.id)
