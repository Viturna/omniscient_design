class QuizzesController < ApplicationController
  ...
  def index
    ...
    if params[:category].present? || params[:domaine_id].present? || params[:count_range].present? || params[:status].present?
      ...
      base_quizzes = base_quizzes.where(domaine_id: params[:domaine_id]) if params[:domaine_id].present?
  ...
  def leaderboard
    ...
    @users = if @leaderboard_type == 'global'
               User.where('total_quiz_points > 0').includes(profile_image_attachment: :blob).order(total_quiz_points: :desc)
             else
               User.where('quiz_points > 0').includes(profile_image_attachment: :blob).order(quiz_points: :desc)
             end

    @users = @users.where('pseudo ILIKE ?', "%#{params[:query]}%") if params[:query].present?
  ...
  def show
    ...
    ref_ids_in_answers = @questions.flat_map(&:quiz_answers).map(&:content).select do |c|
      c.to_s.start_with?('reference_image:')
    end.map { |c| c.split(':').last }
    ...
    @questions_json = @questions.map do |q|
      {
        ...
        quiz_answers: q.quiz_answers.map do |a|
