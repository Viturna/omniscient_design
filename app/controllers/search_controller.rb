class SearchController < ApplicationController
  ...
  def autocomplete
    ...
    return render json: { error: 'Query too short' }, status: :bad_request if query.length < 1
  ...
  def search
    ...
    if query.present?
      ...
      redirect_to studio_path(studio) and return if studio
      if designer && (reference.nil? || query.length <= designer.nom.length)
        redirect_to designer_path(designer) and return
      end

      redirect_to reference_path(reference) and return if reference
    end
    ...
    if params[:notions].present?
      ...
      if notion_ids.any?
        ...
        notion_reference_ids = Reference.joins(:notions).where(notions: { id: notion_ids }).group('references.id').having(
          'COUNT(DISTINCT notions.id) = ?', notion_ids.size
        ).pluck(:id)
